...
const MessagesList = (props) => {
      ...
      try {
          ...
          if (typeof data != "string" && data.length > 0) {
            ...
            data.forEach((item) => {
              ...
              switch (item.message_type) {
                case 3:
                  temp.image = item.message_content;
                  break;
                case 4:
        ...
        socket.on("messageClient", (message) => {
          ...
          switch (message.message_type) {
            case 3:
              temp.image = 'http://172.17.20.131:3000/streaming-file/get-file/' + message.message_content;
  ...
  const onSend = useCallback((message = []) => {
    if (length == 0 && props.value.type == 2) {
      const newMes = {
    ...
    try {
      ...
      if (length == 0 && props.value.type == 2) {
        const newMes = {
          another_user_id: props.value.id,
          chat_message: {
            message_type: checkType,
            message_content: result.name + ";base64;" + fileToBase64,
            //message_time:	new Date(Date.now()),
            chat_room_id: false,
          },
        };
